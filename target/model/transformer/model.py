...
def create_and_train_model(EPOCHS, batch_size, maxlen, input_vocab_size, target_vocab_size, d_model,num_heads, dff,num_layers, x_train, x_valid):
    ...
    def train_step(inp):
        tar_inp = inp[:, :-1] #The first char is the start token - the last the end token
        tar_real = inp[:, 1:]

        enc_padding_mask, combined_mask, dec_padding_mask = create_masks(inp, tar_inp)

        with tf.GradientTape() as tape:
            predictions, _ = transformer(inp, tar_inp, #The inp has to be padded so only the previous AAs are used
                                         True,
                                         enc_padding_mask, 
                                         combined_mask, #Decoder mask
                                         dec_padding_mask)
